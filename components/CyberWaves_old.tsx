...
export function CyberWaves({
  ...
  useEffect(() => {
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (distance < connectionDistance) {
            ...
            // Create gradient line for a more professional look
            const gradient = ctx.createLinearGradient(
              particles[i].x, particles[i].y,
              particles[j].x, particles[j].y
            );
            gradient.addColorStop(0, `rgba(59, 130, 246, ${opacity})`); // Blue
            gradient.addColorStop(0.5, `rgba(99, 179, 237, ${opacity * 0.8})`); // Light blue
    ...
    const animate = () => {
      ...
      // Add subtle scanning lines for cyber effect
      if (time % 200 === 0) {
